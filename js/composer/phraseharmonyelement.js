...
class PhraseHarmonyElement extends PlannedHarmonyElement {
    ...
    getSeed(rnd, localSeed, useLocalSeed) {
    ...
    getBestSuccessor(currentIndices, lengthInfos, beatLengths, totalLength) {
    ...
    getBeatLengthFromIndices(indices, beatLengths) {
        let result = 0;
    ...
    getSinglePenalty(currentValue, wantedValue) {
    ...
    getPenalty(currentIndices, lengthInfos, beatLengths, totalLength) {
        let resultPenalty = 0;
    ...
    getBestIndices(startIndices, lengthInfos, beatLengths, totalLength) {
        ...
        for (let i=0; i<this.maxLengthSearchSteps; i++) {
            ...
            if (successor) {
                let penalty = this.getPenalty(successor, lengthInfos, beatLengths, totalLength);
        ...
        while (replan) {
            ...
            if (!skipInitialTonic) {
                ...
                if (staticIsChromatic) {
                    ...
                    let chromaticGenerator = new ChromaticOscillationHarmonyGenerator(staticOptions);

                    let reuseIndex = JSON.stringify(chromaticGenerator);
                    ...
                    let reuseIndex = JSON.stringify(staticGenerator);
                    let toReuse = module.reusables[reuseIndex];
                    if (toReuse) {
                        staticSolution = copyValueDeep(toReuse);
            ...
            if (!skipDynamicHarmony) {
                ...
                if (dynamicIsChromatic) {
                    ...
                    let chromaticGenerator = new ChromaticTransitionHarmonyGenerator(dynamicOptions);
                    ...
                    //            let dynamicMinorPossibleEndSusRoots = [3, 5];
                    //            let dynamicMajorPossibleEndSusRoots = [1, 3, 5];
                    ...
                    //            let minorModulationPossibleEndSusRoots = [3, 5];
                    //            let majorModulationPossibleEndSusRoots = [1, 3, 5];
                    ...
                    let reuseIndex = JSON.stringify(dynamicGenerator);
                    let toReuse = module.reusables[reuseIndex];
                    let dynamicSolution = null;
                    if (toReuse) {
                        dynamicSolution = copyValueDeep(toReuse);
            ...
            if (!skipDominant) {
                ...
                let lengthenCount = 0;
                if (lengthenDominant) {
                    let rnd2 = new MersenneTwister(dominantCadenceOptions.seed);
                ...
                let reuseIndex = JSON.stringify(dominantCadenceGenerator);
                let toReuse = module.reusables[reuseIndex];
            ...
            if (!skipTonicCadence) {
                ...
                if (lengthenFinalTonic) {
                    let rnd2 = new MersenneTwister(tonicCadenceOptions.seed);
                    lengthenMeasures = getLengthenMeasures(rnd2);
                ...
                let reuseIndex = JSON.stringify(tonicCadenceGenerator);
                let toReuse = module.reusables[reuseIndex];
